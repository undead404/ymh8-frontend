...
export default async function getTagList(
  tagName: string
): Promise<EnhancedWeightedAlbum[]> {
  console.log("getTagList", tagName);
  const topReleases = await database.queryMany(
    ...
    SQL`
        ...
        AND "TagListItem"."tagName" = "AlbumTag"."tagName"
        WHERE "hidden" IS NOT TRUE
        AND "TagListItem"."tagName" = ${tagName}
  ...
  for (let i = 0; i < topReleases.length; i += 1) {
    ...
    enhancedTopReleases.push({
      ...topRelease,
      place: i + 1,
      places: (await getAlbumPlaces(topRelease)).filter(
        ({ tagName: otherTagName }) => tagName !== otherTagName
      ),
      tags: (await getAlbumTags(topRelease)).filter(
        ({ tagName: otherTagName }) => tagName !== otherTagName
      ),
    });
  }
