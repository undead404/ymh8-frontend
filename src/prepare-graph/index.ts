...
import force from "graphology-layout-force"; // <-- Беремо це
...
graph.forEachNode((nodeId) => {
  ...
  topRelations.forEach((rel) => {
    // Нормалізація 0..1
    const normalizedWeight = Math.log(rel.weight + 1) / Math.log(maxWeight + 1);
...
const links = graph.mapEdges((key, attributes, source, target) => {
  return {
    source: source,
    target: target,
...
await writeFile("./src/data/graph/data.json", JSON.stringify(finalData));
console.log("✅ Готово! Файл збережено в src/data//graph/data.json");

const byCommunity = groupBy(nodes, (node) => (node as any).community);

for (const communityIndex in byCommunity) {
  const community = byCommunity[communityIndex].toSorted(
    (a, b) => (b as any).originalPopularity - (a as any).originalPopularity
  );
  console.log(communityIndex);
  for (const communityMember of community) {
    console.log(communityMember.id);
  }
  console.log("\n");
  // await writeFile(
  //   `./src/data/clusters/${communityIndex}.json`,
  //   JSON.stringify({
  //     members: community,
  //     name: `${community.at(0)!.id} and friends`,
  //   })
  // );
}
